---
name: rcmd
description: Run commands on remote telnet, ssh, serial, serial_bridge, prompt/prompt_bridge or adb devices (embedded boards, dev servers) with stateful sessions, accurate exit codes, and file push/pull, via the `rcmd` CLI. Use whenever a task involves executing shell commands on a remote device over telnet/ssh/serial/serial_bridge/prompt/adb — inspecting logs, checking status, deploying or replacing binaries on a board, running scripts on a board or server. serial_bridge reaches a serial console over a serial-bridge WebSocket gateway on another host; prompt/prompt_bridge drive prompt-only shells with no $? (RT-Thread msh/FinSH, U-Boot) using the shell prompt as the command boundary. Prefer this over raw `ssh host cmd` (no state), `sshpass`, or `tmux send-keys` (no exit code). Devices are pre-configured by name in devices.yaml; the skill ships alongside the rcmd tool.
---
...
`prompt` / `prompt_bridge` 用于 **无 `$?` 的提示符型 shell**(RT-Thread msh/FinSH、U-Boot)——这类 shell 没有 `$?`/分号语法，用 bash 哨兵的 `serial`/`ssh` 会超时。它们改用**提示符重现**作为命令边界(正则,`prompt:` 每设备可配,默认 RT-Thread msh),退出码为启发式(输出命中 `error_pattern:` 默认 `command not found` → 127,否则 0)。`prompt` 走本地串口,`prompt_bridge` 走 serial-bridge 网关。给嵌入式板的 msh 控制台发命令用它,而不是 `serial`。

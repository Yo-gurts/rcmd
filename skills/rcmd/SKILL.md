---
name: rcmd
description: Run commands on remote telnet or ssh devices (embedded boards, dev servers) with stateful sessions and accurate exit codes, via the `rcmd` CLI. Use whenever a task involves executing shell commands on a remote device over telnet/ssh — inspecting logs, checking status, running scripts on a board or server. Prefer this over raw `ssh host cmd` (no state), `sshpass`, or `tmux send-keys` (no exit code). Devices are pre-configured by name in ~/rcmd/devices.yaml.
---

# rcmd — 远程 telnet/ssh 命令

`rcmd` 让你在远程 telnet/ssh 设备上跑命令，像本地一样：**有状态**（`cd`/env 跨调用保留）、**退出码准确透传**、telnet 与 ssh **统一接口**。已在 `PATH` 中，直接调用 `rcmd`。
...
设备定义在 `~/rcmd/devices.yaml`（含密码，已 gitignore；模板见 `devices.yaml.example`）。加新设备就在该文件加一个命名块（transport/host/port/username/password，telnet 另需 login_prompt/password_prompt）。环境变量：`RCMD_TIMEOUT`（每条命令超时秒数）、`RCMD_CONFIG`（配置路径）。
